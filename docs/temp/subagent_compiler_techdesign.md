...
3. **最小侵入集成**：复用现有 Tool、MCP 与 ReasoningEngine 体系，skills 作为提示词约束。
...
- **配置驱动**：通过 SubAgentConfig 限制工具集合，skills 作为提示词约束注入系统提示词。
...
1. 最佳实践文档（Markdown）
2. 系统工具清单（Tools/MCP）与 skills 提示词约束
3. 编译选项（模式、约束、默认策略）
...
- 工具是否存在于 ToolCatalog/MCP，skills 是否存在于 SkillManager
- 提示词长度预算
- 输出格式完整性
- 配置版本与兼容性
...
| tools | 可执行工具集合（ToolCatalog + MCP，空值时使用默认核心工具 + 技能工具集合） |
| prompt_policies | skills 作为提示词侧约束，通过技能工具调用 |
...
- 动态注入“前置步骤输出”
- 注入工具清单与 skills 约束以匹配能力边界
- skills 的具体执行依赖 get_skill_info / run_skill_script
...
- 与 MainAgent 相同的执行链路
- 仅向模型暴露配置中声明的 tools/mcp_tools
- 未配置 tools/mcp_tools 时，使用默认核心工具与技能工具集合（list_skills / get_skill_info / run_skill_script / install_skill / load_skill / reload_skill 等）
